    ...
    <section class="content-header">
        <div class="container-fluid">
            @include('partials.header-breadcrumbs', [
                ...
                'breadcrumbs' => [['label' => 'List all locations', 'active' => true]],
            ])
        </div><!-- /.container-fluid -->
    </section>
    ...
    <section class="content">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">List all locations</h3>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @forelse ($locations as $location)
                            <td>{{ $location->project->company->company_name }}</td>
                            <td>{{ $location->project->name }}</td>
                            <td>{{ $location->name }}</td>
                            <td>{{ $location->latitude }}</td>
                            <td>{{ $location->longitude }}</td>
                            <td>{{ $location->radius }}</td>
                            <td> Created : {{ $location->created_at->format('d.m.Y H:i') }}
                                @if ($location->updated_at != $location->created_at)
                                    <br>Updated : {{ $location->updated_at->diffforHumans() }}
                                @elseif ($location->deleted_at)
                                    <br>Deleted : {{ $location->deleted_at->diffforHumans() }}
                                @endif
                            </td>
                            <td>{{ $location->employees->pluck('employee_name')->implode(', ') }}</td>
                            @role('Super Admin')
                                <td>
